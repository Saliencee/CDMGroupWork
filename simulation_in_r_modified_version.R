...
generate_coherent_name_and_gender <- function(country) {
  ...
  return(list(name = paste(first_name, last_name, sep=" "), gender = gender))
}
...
generate_coherent_bmi_and_height <- function(age) {
  height <- round(pmax(pmin(rnorm(1, 170 - ifelse(age < 20, 50, 0), 10), 200), 150),1)
...
generate_snp_values <- function() {
  return(sample(c(0, 1, 2), 5, replace=TRUE, prob=c(0.25, 0.5, 0.25)))
}
